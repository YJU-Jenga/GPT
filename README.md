...
1. 시작하기 전에…
    1. 이 글은 2023년 6월에 작성된 글로 차후 API 변경 등의 외부 요소의 변경이 있을 경우 작동하지 않을 수 있습니다.
...
5. 소프트웨어 셋팅
    - screen을 활용해서 알람 python파일 실행하기
        1. SSH로 라즈베리 파이에 접속합니다.
        2. 터미널에서 다음 명령어를 사용하여 스크린을 설치합니다:
        ...
        4. 이제 스크린 세션 내에서 원하는 작업을 실행할 수 있습니다. 예를 들어, .py 파일을 실행하거나 명령어를 실행할 수 있습니다.
            
            ```bash
            $ python3 jwt_token.py
            ```
            
        5. 작업을 백그라운드로 전환하려면 다음 단축키를 사용합니다: **`Ctrl + A`**, 그리고 **`d`**를 순서대로 누릅니다. 이렇게 하면 스크린 세션은 백그라운드로 전환됩니다.
        6. 스크린 세션을 다시 가져오려면 다음 명령어를 사용합니다:만약 여러 개의 스크린 세션이 있을 경우, 스크린 세션을 선택할 수 있는 목록이 표시됩니다. 해당 세션을 선택하여 가져올 수 있습니다.
            
            ```bash
            screen -r
            ```
            
        7. 스크린 세션을 완전히 종료하려면 세션 내에서 다음 명령어를 사용합니다:
        ...
        이렇게 하면 스크린을 사용하여 백그라운드에서 작업을 실행하고, 세션을 전환하고, 세션을 종료할 수 있습니다.
...
6. 운영체제 기본 설정
    - **주의) AI 코디니에서 제공하는 운영체제는 Raspbian Buster를 AI 코디니에서 수정을 가한 운영체제이므로 apt를 통해  패키지 업그레이드를 할 경우 AI 코디니에서 수정해 두었던 기본 설정이 꼬일 가능성이 있으므로 apt를 통해 패키지 업그레이드를 권장하지 않으며 AI코디니에서 제공하는 업데이트만 받는 다는 전제하에 설명한다.**
    - **주의) 파이썬 버전은 3.7 버전을 사용하며 이외의 버전에서 정상 작동을 보장하지 않는다.**
    - **주의) 다른 GPIO 확장 킷(터치 디스플레이 등)을 사용함으로써 생기는 문제는 사용자가 직접 해결해야 한다.**
    - **주의) AI 코디니에서 제공하는 운영체제의 사용자 ID는 pi, 초기 비밀번호는 kt123!@#이다.**
    ...
    - SD카드 확장하기
        - 16기가 이상의 microSD를 사용 중이라면  microSD 카드의 모든 용량을 사용하기 위해서 확장 작업이 필요하다.
            ...
            ![Screenshot 0005-06-19 at 19.31.19.png](README_PHOTO/Screenshot_0005-06-19_at_19.31.19.png)
    ...
    - ssh 개방
        - 내부망에서만 사용하는 것을 상정하여 설명하는 것 이기에 포트 개방 등의 설정은 설명하지 않는다.
        - 터미널을 열어서 아래의 명령어를 입력한다.
        ...
        - 터미널 창에서 ifconfig를 입력하여 wlan0의 IP주소를 확인한다.
            - 연결 환경에 따라 다를 수도 있다. ex) RJ45 랜선을 연결 한 경우 eth0의 IP 주소를 확인해야 한다.
            - 아래 예시 참고
        ...
        - Mac, Linux에서 ssh를 통해 라즈베리파이 터미널 접속하기
            - 아래의 예시 참고
                ...
                kimjongryul@byungshin$ ssh pi@192.168.0.29 -p 6974
    ...
    - [config.py](http://config.py)에 다음과 같은 코드를 작성한다.
        ...
        - api 발급 방법은 아래의 방법을 따른다.
            - openai_api_key 발급 가이드
                ...
                Option) 위의 단계는 대략적인 가이드일 뿐이며, OpenAI의 정책이나 절차가 변경될 수 있으므로 실제 절차는 OpenAI 웹사이트에서 확인하시는 것이 좋습니다.
                
                Option) OpenAI의 공식 문서와 지원 채널은 API 사용에 대한 자세한 정보와 도움을 제공할 수 있습니다.
            ...
            - youtube_api_key 발급 가이드
                ...
                5. API 키 가져오기: API 키가 생성되면, 해당 키를 안전한 곳에 저장하거나 복사해둔다. 이 API 키를 사용하여 YouTube API에 접근하고 요청을 보낼 수 있다.
                    ...
                    Option) 위의 절차는 YouTube API 키를 발급받는 대략적인 가이드이며, Google의 정책이나 절차가 변경될 수 있다. 
                    
                    Option) 실제 절차는 Google Cloud Console에서 확인하시는 것이 좋다. 
                    
                    Option) Google Cloud Console의 문서와 지원 채널은 API 키 발급과 관련된 자세한 정보와 도움을 제공하므로 필요시 참고하는 것이 좋다.
                    
7. 인형에 넣기
- 라즈베리 파이와 배터리를 인형 내부에 넣기 위해서는 발열 관리와 쇼트가 나지 않도록 배선 작업에 유의해야 합다. 아래는 각 작업에 대한 고려사항이다.
    1. 쓰로틀링을 막기 위한 발열 관리:
